...
def dump_bin(ncfiles, varname):
    meters2km = 1.e-3
    '''
    Need to loop over each file in the list of netCDF files to be able to
    construct multiple vdfs
    '''
    for ncfile in ncfiles:
        with Dataset(ncfile, 'r') as nc_in:
            try:
                ...
                # tvals = nc_in.variables['time'][:]
                filenames = ['xvals.txt', 'yvals.txt', 'zvals.txt']  # , 'tvals.txt']
                arrays = [xvals, yvals, zvals]  # , tvals]
...
def dump_script(varname, rev_shape, time_s):
    command = r"""
        #!/bin/bash -v
