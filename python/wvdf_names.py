...
def dump_script(varname, rev_shape, outname):
    command = r"""
        #!/bin/bash -v
        ...
        vdfcreate  -xcoords xvals.txt -ycoords yvals.txt -zcoords zvals.txt \
           -gridtype stretched -dimension {dim:s} -vars3d {var:s} -numts 1 {outn:s}.vdf

        raw2vdf -varname {var:s} -ts 0 {outn:s}.vdf {outn:s}.bin
    """

    vars = dict(var=varname, dim=rev_shape, outn=outname)
...
if __name__ == "__main__":
    ...
    binfile, rev_shape = dump_bin(args.ncfile, args.varname, args.outname)
    dump_script(args.varname, rev_shape, args.outname)